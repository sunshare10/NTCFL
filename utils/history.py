...
class History():
    ...
    def fedafter_epoch(self,meta):
        '''
        保存每周期的 'Train Loss', 'Val Acc', 'Precision', 'Recall', 'F1 Score'
        '''
        for idx in range(meta['num_users']):
            acc_epoch = []
            epoch_outputs = []
            csv_name='client'+str(idx)+'_metrics_outputs.csv'
            pic_name='client'+str(idx)+'_loss-acc.png'
            dir=meta['save_dir']
            csv_dir=os.path.join(dir,csv_name)
            pic_dir = os.path.join(dir,pic_name)
            with open(csv_dir, 'w', newline='') as f:
                writer = csv.writer(f)
                for i in range(len(meta['train_info'+str(idx)]['train_loss'])):
                    temp_data = [i+1, meta['train_info'+str(idx)]['train_loss'][i], meta['train_info'+str(idx)]['val_acc'][i].get('accuracy_top-1'),mean(meta['train_info'+str(idx)]['val_acc'][i].get('precision',0.0)),mean(meta['train_info'+str(idx)]['val_acc'][i].get('recall',0.0)),mean(meta['train_info'+str(idx)]['val_acc'][i].get('f1_score',0.0))]
                    acc_epoch.append(meta['train_info'+str(idx)]['val_acc'][i].get('accuracy_top-1'))
                    epoch_outputs.append(temp_data)
                writer.writerows(epoch_outputs)
            ...
            绘制每周期Train Loss以及Validation Accuracy
            '''
            total_epoch = range(1,len(meta['train_info'+str(idx)]['train_loss'])+1)

            fig, ax1 = plt.subplots()
            color = 'tab:red'
            ax1.set_xlabel('Epoch')
            ax1.set_ylabel('Loss')
            ax1.plot(total_epoch, meta['train_info'+str(idx)]['train_loss'], 'red', linewidth = 2, label='Train loss')
            ax1.grid(True)
