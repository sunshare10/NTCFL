...
def trainfed(idx,model, runner, lr_update_func, device, epoch, epoches, meta,trainloader,jilu=False):
    train_loss = 0
    
    meta['epoch'] = runner['epoch']
    # print(runner.get('optimizer').cuda())
    model.train()
    model.to(device)
    with tqdm(total=len(trainloader),desc=f'Train client {idx}: Epoch {epoch + 1}/{epoches}',postfix=dict,mininterval=0.3) as pbar:
        for iter, batch in enumerate(trainloader):            
            images, targets = batch
            with torch.no_grad():
                images  = images.to(device)
                targets = targets.to(device)
            # runner.get('optimizer').to(device)
            ...
            pbar.update(1)
    if jilu:
        meta['train_info'+str(idx)]['train_loss'].append(train_loss / (iter + 1))
                
        if train_loss/len(trainloader) < runner.get(str(idx)+'_best_train_loss',100) :
            runner[str(idx)+'_best_train_loss'] = train_loss/len(trainloader)
            meta['best_train_loss'] = runner[str(idx)+'_best_train_loss']
            if runner.get(str(idx)+'_best_train_weight','#')!='#':
                if epoch > 0 and os.path.isfile(runner[str(idx)+'_best_train_weight']):
                    os.remove(runner[str(idx)+'_best_train_weight'])
            runner[str(idx)+'_best_train_weight'] = os.path.join(meta['save_dir'],'client'+str(idx)+'_Train_Epoch{:03}-Loss{:.3f}.pth'.format(epoch+1,train_loss / len(runner.get('train_loader'))))
            meta['best_train_weight'] = runner[str(idx)+'_best_train_weight']
            save_checkpoint(model,runner.get(str(idx)+'_best_train_weight'),runner.get('optimizer'),meta)
        
    return model.state_dict(), train_loss / (iter + 1)
...
def validationfed(idx,model, runner, cfg, device, epoch, epoches, meta,testloader):
    preds,targets = [],[]
    model.eval()
    with torch.no_grad():
        with tqdm(total=len(testloader), desc=f'Test client {idx}: Epoch {epoch + 1}/{epoches}',mininterval=0.3) as pbar:
    ...
    meta['train_info'+str(idx)]['val_acc'].append(eval_results)
    ...
    if eval_results.get('accuracy_top-1') > runner.get(str(idx)+'_best_val_acc',0):
        runner[str(idx)+'_best_val_acc'] = eval_results.get('accuracy_top-1')
        meta['best_val_acc'] = runner[str(idx)+'_best_val_acc']
        if runner.get(str(idx)+'_best_val_weight','#')!='#':
            if epoch > 0 and os.path.isfile(runner[str(idx)+'_best_val_weight']):
                os.remove(runner[str(idx)+'_best_val_weight'])
        runner[str(idx)+'_best_val_weight'] = os.path.join(meta['save_dir'],'client'+str(idx)+'_Val_Epoch{:03}-Acc{:.3f}.pth'.format(epoch+1,eval_results.get('accuracy_top-1')))
        meta['best_val_weight'] = runner[str(idx)+'_best_val_weight']
        save_checkpoint(model,runner.get(str(idx)+'_best_val_weight'),runner.get('optimizer'),meta)
    if runner.get(str(idx)+'_last_weight','#')!='#':
        if epoch > 0 and os.path.isfile(runner[str(idx)+'_last_weight']):
            os.remove(runner[str(idx)+'_last_weight'])
    runner[str(idx)+'_last_weight'] = os.path.join(meta['save_dir'],'client'+str(idx)+'_Last_Epoch{:03}.pth'.format(epoch+1))
    meta['last_weight'] = runner[str(idx)+'_last_weight']
    save_checkpoint(model,runner.get(str(idx)+'_last_weight'),runner.get('optimizer'),meta)
